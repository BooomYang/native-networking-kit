...
本文记录 iOS L3 loopback integration harness。三端通用测试分层见 `docs/testing-strategy.md`。

## 定位

`./scripts/verify-ios-network-harness.sh` 验证真实 `URLSession`、`URLSessionNativeHttpEngine` 和 `127.0.0.1` mock server 的端到端 adapter 路径。它不是 unit test，不访问 public network。
...
HTTP non-2xx 语义主验证放在 L2 adapter unit tests，不放 L3。
