...
Router 默认返回成功，不因为缺少人工确认而 fail。它只统计 active review threads，跳过 `DISMISSED` review、归属 `DISMISSED` review 的 inline comment 和自身生成的 summary comment，避免旧 P1/P2 结论反复污染结果。`pull_request` 事件只做只读分类和 Actions step summary；`pull_request_target` 或 `issue_comment` 事件才尝试写入 PR labels/comment。它只负责把有限注意力推给真正敏感的 PR。
